...
<template>
  <section class="workspace job-workspace">
    ...
    <div v-if="job.status === 'NEEDS_USER_ACTION'" class="qr-login-banner">
      ...
      <div class="qr-copy"><span class="banner-icon"><QrCode :size="22" /></span><div><strong>请登录飞书工作台</strong><p>{{ job.currentAction }}</p><small>请在已打开的飞书授权页面完成扫码，完成后返回此页面继续任务。</small></div></div>
